...
- Per-session worktrees: `n` creates a `claude/<8hex>` worktree+branch instantly at the cursor (no dialog); branch renames to `claude/<slug>` on first auto-title via `git branch -m` (worktree dir name stays `<repo>-claude-<8hex>` forever). Sidebar groups worktrees under their main repo via `session.GetMainRepo`; each worktree-session row shows its own branch + dirty + PR badge alongside the group header's main-repo info. Fleet never pushes or opens MRs — manual.
